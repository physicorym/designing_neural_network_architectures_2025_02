...
Разметьте область номерного знака на каждом изображении (bounding box).
...
### 1.3. Подготовка структуры проекта

Организуйте директории, например:
...
Обучите модель детекции на своей разметке (например, `YOLOv8n` или другой детектор из семинара).
...
- На каких кейсах модель ошибается чаще всего.
